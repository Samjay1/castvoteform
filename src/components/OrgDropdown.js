const OrgDropdown = ({defaultValue,onValueChange,errorState,question,questionName,answersDisplay,answers})=>{

    const answerList = answers.map((answer,index)=> {
        let value = answer;
        // console.log('value, defaultValue :>> ', value, defaultValue);
           return <option  key={index} value={value} >{answersDisplay[index]}</option>
